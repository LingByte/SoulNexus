...
**在线体验 SoulNexus**: [https://lingecho.com](https://lingecho.com)
...
SoulNexus 灵枢是一个基于 Go + React 的企业级智能语音交互平台，为用户提供完整的AI语音交互解决方案。集成了先进的语音识别（ASR）、语音合成（TTS）、大语言模型（LLM）和实时通信技术，支持实时通话、声音克隆、知识库管理、工作流自动化、设备管理、告警、账单、组织管理等企业级功能。
...
<div align="center">
  <img src="docs/core.png" alt="SoulNexus 核心架构" width="800">
</div>
...
# 启动语音服务（SIP / WebSocket-Web / WebSocket-Hardware / WebRTC）
cd server
cp .env-voice.example .env-voice    # 首次运行：填入 ASR / TTS 凭据
go run ./cmd/voice                  # 监听 VOICE_SIP_ADDR + VOICE_HTTP_ADDR
```

**语音服务传输通道**（统一由 `cmd/voice` 提供）：

| 通道 | 端点 | 开关 |
| --- | --- | --- |
| SIP（信令 + RTP） | `VOICE_SIP_ADDR`、`VOICE_RTP_START..END` | 默认开启 |
...
| WebSocket SoulNexus-HW | `VOICE_HTTP_ADDR` + `VOICE_SOULNEXUS_HW_WS_PATH` | 路径非空即启用 |

完整环境变量见 `.env-voice.example`。
