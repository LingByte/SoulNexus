...
const Badge = ({
  ...
  return (
    ...
    >
      ...
      {/* 如果有icon prop，使用原来的布局；否则直接渲染children（支持flex-col） */}
      {icon ? (
        <div className={cn('relative flex', isVertical ? 'flex-col items-center gap-1' : 'items-center gap-1.5')}>
          <motion.span 
        ...
        <div className={cn('relative', isVertical ? 'flex flex-col items-center gap-1' : 'flex items-center gap-1.5')}>
          {children}
        </div>
      )}
    </motion.span>
