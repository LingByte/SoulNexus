...
// DIALOG_AGENT_ID into the -dialog-ws URL query string so operators can
// keep secrets out of argv and load them from the environment (or .env)
// instead. Non-empty env values overwrite existing query keys with the
// same name so a single base URL can stay stable while credentials rotate.
...
func warnDialogAuthIfNeeded(dialogURL string) {
	...
	log.Printf("[warn] dialog-plane URL still missing apiKey, apiSecret, or agentId after merging DIALOG_* env. " +
		...
		"WebRTC: POST these fields in /webrtc/v1/offer JSON from the browser; SIP/xiaozhi: set DIALOG_* env or add query params to -dialog-ws.")
}
