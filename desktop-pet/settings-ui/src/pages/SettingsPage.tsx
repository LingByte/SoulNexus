...
import { WidgetMarketPanel } from '@/components/widget-market-panel'
...
export default function SettingsPage() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center gap-2 text-sm text-muted-foreground bg-background">
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <header className="sticky top-0 z-10 shrink-0 border-b border-border bg-card/95 backdrop-blur-md px-5 py-3.5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-semibold tracking-tight text-foreground">SoulMy 设置</h1>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button
              ...
              size="sm"
              className={cn(
                'h-8 gap-1.5 text-xs border-primary/30 bg-background hover:bg-primary/10 hover:text-primary',
                marketOpen && 'bg-primary/15 text-primary border-primary/50',
              )}
            ...
            <span
              className={cn(
                ...
                configured
                  ...
                  : 'bg-muted text-amber-200 border-amber-500/40',
              )}
            >
              {configured ? (
                <>
      ...
      <div className="flex flex-1 min-h-0">
        <PetWarehouseSidebar
          ...
          primaryPetId={form.primaryPetId}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <main className="flex-1 overflow-y-auto space-y-3 p-4 pb-4 min-w-0 max-w-2xl">
          {pet ? (
            <Card className="border-dashed border-primary/20 bg-primary/[0.03]">
              <CardContent className="flex items-center justify-between gap-3 py-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate text-foreground">正在编辑 · {pet.name}</p>
                  <p className="text-[11px] text-muted-foreground font-mono truncate mt-0.5">{pet.jsSourceId}</p>
                </div>
                <Button
                  type="button"
                  size="sm"
          ...
          {status.msg ? (
            <div
              className={cn(
                ...
                status.kind === 'ok' && 'border-emerald-200/80 bg-card text-emerald-800',
                status.kind === 'err' && 'border-red-200/80 bg-card text-red-800',
          ...
          <footer className="shrink-0 border-t border-border bg-card/95 backdrop-blur-md px-4 py-3">
