...
//   - 失败 5 次锁 5 分钟由 authmodel.IncrementTwoFAFailedAttempts/ResetTwoFAFailedAttempts 管理。
...
func twoFAHelperGetOrSync(h *Handlers, user *authmodel.User) (*authmodel.TwoFA, error) {
	t, err := authmodel.GetTwoFAByUserID(h.db, user.ID)
	if err != nil {
		return nil, err
	}
	if t != nil {
	...
	row, err := authmodel.UpsertTwoFASecret(h.db, user.ID, user.TwoFactorSecret)
	if err != nil {
		return nil, err
	}
	if user.TwoFactorEnabled {
		_ = authmodel.EnableTwoFA(h.db, user.ID)
		row.IsEnabled = true
	}
	return row, nil
}
...
func (h *Handlers) handleMeTwoFAStatus(c *gin.Context) {
	...
	if t != nil {
		...
		if cnt, err := authmodel.CountUnusedBackupCodes(h.db, user.ID); err == nil {
...
func (h *Handlers) handleMeTwoFAReset(c *gin.Context) {
	...
	// 同步 User 兼容字段。
	_ = h.db.Model(&authmodel.User{}).Where("id = ?", user.ID).Updates(map[string]interface{}{
