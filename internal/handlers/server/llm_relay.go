...
func relayTokenFromRequest(db *gorm.DB, c *gin.Context) (*svcmodels.LLMToken, bool) {
	key := extractRelayAPIKey(c)
	if key == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": gin.H{"type": "auth_error", "message": "missing API key"}})
		return nil, false
	}
	t, err := svcmodels.LookupActiveLLMToken(db, key)
	if err != nil || t == nil {
...
func relayChannelsForModel(db *gorm.DB, group, model, protocol string, orgID uint) ([]svcmodels.LLMChannel, error) {
	...
	var ab []abilityRow
	abQ := db.Model(&svcmodels.LLMAbility{}).
		Select("channel_id").
		Where("`group` = ? AND model = ? AND enabled = ?", group, model, true)
	abQ = svcmodels.ScopeOrgVisible(abQ, orgID)
	...
	q := db.Model(&svcmodels.LLMChannel{}).Where("id IN ? AND status = ? AND protocol = ?", ids, 1, protocol)
	q = svcmodels.ScopeOrgVisible(q, orgID)
...
func toUpstreamChannel(ch *svcmodels.LLMChannel) llm.UpstreamChannel {
...
func computeQuotaDelta(db *gorm.DB, model string, prompt, completion int) int {
	var meta svcmodels.LLMModelMeta
...
func relayMeta(c *gin.Context, t *svcmodels.LLMToken) llm.RelayUsageMeta {
	uid := ""
	if t != nil && t.UserID > 0 {
...
func (h *Handlers) preflightTokenForModel(c *gin.Context, t *svcmodels.LLMToken, model string) bool {
	if exceeded, why := t.QuotaExceeded(); exceeded {
		...
		go func(userID uint, name, reason string) {
			...
			_ = svcmodels.NewInternalNotificationService(h.db).Send(
				userID,
				"LLM Token 调用被拦截",
...
func requireTokenType(c *gin.Context, tok *svcmodels.LLMToken, want svcmodels.LLMTokenType) bool {
	if tok == nil {
		return false
	}
	got := tok.Type
...
func (h *Handlers) handleRelayOpenAIChat(c *gin.Context) {
	...
	if !requireTokenType(c, tok, svcmodels.LLMTokenTypeLLM) {
		return
	}
	...
	_ = svcmodels.AddLLMTokenUsage(h.db, tok.ID, tt, quota)
}

// relayMultiOpenAINonStream 按 priority 顺序遍历 channels，第一个 BusinessOK 即返回。
func relayMultiOpenAINonStream(ctx context.Context, body []byte, accept string, channels []svcmodels.LLMChannel) *llm.RelayResult {
	t0 := time.Now()
	res := &llm.RelayResult{}
	for i := range channels {
...
func relayMultiAnthropicNonStream(ctx context.Context, body []byte, anthropicVer, anthropicBeta string, channels []svcmodels.LLMChannel) *llm.RelayResult {
	t0 := time.Now()
	res := &llm.RelayResult{}
	for i := range channels {
...
//
// 解析顺序见 svcmodels.BuildOpenAPIModelListForToken：
...
func (h *Handlers) handleRelayOpenAIModelsList(c *gin.Context) {
	...
	items, err := svcmodels.BuildOpenAPIModelListForToken(h.db, tok)
	if err != nil {
