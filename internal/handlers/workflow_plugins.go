...
func (h *Handlers) registerWorkflowPluginRoutes(r *humax.Group) {
	...
	plugins := r.Group("workflow-plugins")
	{
		...
		plugins.GET("/workflow/:workflowId/published", pluginHandler.GetWorkflowPublishedPlugin)
		plugins.GET("/installed", pluginHandler.ListInstalledWorkflowPlugins)
		plugins.GET("/my-plugins", pluginHandler.GetUserWorkflowPlugins)
	}

	pluginsAuth := r.Group("workflow-plugins")
	pluginsAuth.Use(middleware.RequireHumanJWTUser())
	{
...
func (h *WorkflowPluginHandler) GetWorkflowPlugin(c *gin.Context) {
	...
	if err := h.db.Preload("Versions").Preload("Reviews").
