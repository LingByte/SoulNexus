...
func (h *Handlers) registerWorkflowRoutes(r *humax.Group) {
	workflows := r.Group("/workflows")
	{
		...
		instances.GET("", h.ListWorkflowInstances)
		instances.GET("/export", h.ExportWorkflowInstances)
		instances.GET("/:id", h.GetWorkflowInstance)
	}

	write := r.Group("/workflows")
	write.Use(middleware.RequireHumanJWTUser())
	{
		defs := write.Group("/definitions")
		...
		defs.POST("/:id/publish", h.PublishWorkflowDefinition)
		defs.POST("/:id/versions/:versionId/rollback", h.RollbackWorkflowVersion)
		inst := write.Group("/instances")
		inst.POST("/:id/stop", h.StopWorkflowInstance)
	}
}
...
func (h *Handlers) CreateWorkflowDefinition(c *gin.Context) {
	...
	if slug == "" {
		slug = ensureUniqueWorkflowSlug(h.db, input.Name)
	}
...
func (h *Handlers) RunWorkflowDefinition(c *gin.Context) {
	...
	workflowdef.AttachRuntimeContext(runtimeWf.Context, def.GroupID, workflowdef.NewServiceKnowledgeRecaller(h.db, h.kb))
	...
	runMeta := workflowInstanceMetaFromContext(c, &def, user, "manual", input.Parameters)
	workflowdef.FinalizeInstance(&instance, runtimeWf.Context, execErr, runMeta)
...
func (h *Handlers) TestWorkflowNode(c *gin.Context) {
	...
	var input struct {
		Parameters map[string]interface{}      `json:"parameters"`
		Node       *models.WorkflowNodeSchema  `json:"node"`
		Edges      []models.WorkflowEdgeSchema `json:"edges"`
	}
	if err := c.ShouldBindJSON(&input); err != nil {
	...
	// Find the node: prefer editor draft payload, fallback to saved definition
	var targetNode *models.WorkflowNodeSchema
	edges := def.Definition.Edges
	if input.Node != nil && input.Node.ID != "" {
	...
	ctx := runtimewf.NewWorkflowContextWithDB(fmt.Sprintf("test-node-%s", nodeID), h.db)
	workflowdef.AttachRuntimeContext(ctx, def.GroupID, workflowdef.NewServiceKnowledgeRecaller(h.db, h.kb))
	...
	normalizeNodeTestParameters(targetNode, ctx)
	...
	runtimeNode, err := workflowdef.BuildRuntimeNode(targetNode, &def.Definition, h.db)
	if err != nil {
		response.Fail(c, "failed to build node", gin.H{"reason": err.Error()})
		return
	}
...
// normalizeNodeTestParameters maps ad-hoc test inputs (e.g. input-0) to configured variable names.
func normalizeNodeTestParameters(node *models.WorkflowNodeSchema, ctx *runtimewf.WorkflowContext) {
	if node == nil || ctx == nil || ctx.Parameters == nil {
		return
	}
	inputVar := strings.TrimSpace(node.Properties["inputVariable"])
	switch node.Type {
	case "knowledge_base":
		if inputVar == "" {
			inputVar = "query"
	...
	if _, ok := ctx.Parameters[inputVar]; ok {
		return
	}
	for key, val := range ctx.Parameters {
		if strings.HasPrefix(key, "_") || key == "parameters" {
			continue
		}
		ctx.Parameters[inputVar] = val
		if key != inputVar {
			delete(ctx.Parameters, key)
		}
		break
	}
}

// PublishWorkflowDefinition snapshots the current draft as an immutable published version.
func (h *Handlers) PublishWorkflowDefinition(c *gin.Context) {
	user := workflowCurrentUser(c)
	if user == nil {
		response.Fail(c, "unauthorized", "User not logged in")
		return
	...
	snapshot := models.WorkflowVersion{
		DefinitionID:     def.ID,
		Version:          def.Version,
		Name:             def.Name,
		Slug:             def.Slug,
	...
	if snapshot.ChangeNote == "" {
		snapshot.ChangeNote = fmt.Sprintf("发布版本 v%d", def.Version)
	}
	if err := h.db.Create(&snapshot).Error; err != nil {
		response.Fail(c, "failed to publish workflow version", err.Error())
		return
	}

	h.recordOpChange(c, OpLogEntry{
		TenantID: middleware.CurrentTenantID(c), Action: constants.OpActionPublish,
		Resource: constants.OpResourceWorkflow, ResourceID: def.ID, ResourceName: def.Name,
		Summary: fmt.Sprintf("Published workflow %s v%d", def.Name, def.Version),
	}, nil, snapshot)

	response.Success(c, "workflow version published", gin.H{
		"definition": def,
		"version":    snapshot,
	})
}
