...
func FinalizeAccountDeletion(db *gorm.DB, userID uint, operator string) error {
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Table("group_members").Unscoped().Where("user_id = ?", userID).Delete(nil).Error; err != nil {
			return err
		}
		if err := tx.Table("group_invitations").Unscoped().Where("inviter_id = ? OR invitee_id = ?", userID, userID).Delete(nil).Error; err != nil {
			return err
		}
