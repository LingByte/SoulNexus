...
func (legacyCredRow) TableName() string { return (&auth.UserCredential{}).TableName() }

func migrateUserCredentialsTenancy(db *gorm.DB, personalOf func(uint) uint) error {
	if !db.Migrator().HasTable((&auth.UserCredential{}).TableName()) {
		return nil
	}
	...
	for _, r := range rows {
		...
		if err := db.Model(&auth.UserCredential{}).Where("id = ?", r.ID).Updates(map[string]interface{}{
			"group_id":   gid,
			"created_by": r.UserID,
		}).Error; err != nil {
			return err
