...
func setupGroupsTestDB(t *testing.T) *gorm.DB {
	...
	err = db.AutoMigrate(&auth.User{}, &UserProfile{}, &Group{}, &GroupMember{}, &GroupInvitation{})
	require.NoError(t, err)

	return db
}

func createTestUserForGroups(t *testing.T, db *gorm.DB, email, username string) *auth.User {
	user := &auth.User{
		Email:    email,
...
func TestGroupMember_CRUD(t *testing.T) {
	...
	err = db.Preload("auth.User").Preload("Group").First(&retrieved, groupMember.ID).Error
	assert.NoError(t, err)
