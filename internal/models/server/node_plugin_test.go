...
func setupNodePluginTestDB(t *testing.T) *gorm.DB {
	...
	// 迁移依赖表
	err = db.AutoMigrate(&auth.User{}, &UserProfile{}, &Group{})
	require.NoError(t, err)

	return db
}

func createTestUserForNodePlugin(t *testing.T, db *gorm.DB) *auth.User {
	user := &auth.User{
...
func TestNodePluginInstallation_CRUD(t *testing.T) {
	...
	err = db.Preload("auth.User").Preload("Plugin").First(&retrieved, installation.ID).Error
	assert.NoError(t, err)
