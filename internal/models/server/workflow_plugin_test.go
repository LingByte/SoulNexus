...
func setupWorkflowPluginTestDB(t *testing.T) *gorm.DB {
	...
	// 迁移依赖表
	err = db.AutoMigrate(&auth.User{}, &UserProfile{}, &Group{}, &GroupMember{}, &WorkflowDefinition{})
	require.NoError(t, err)

	return db
}

func createTestUserForWorkflowPlugin(t *testing.T, db *gorm.DB) *auth.User {
	user := &auth.User{
...
func createTestWorkflowDefinition(t *testing.T, db *gorm.DB, user *auth.User) *WorkflowDefinition {
...
func TestWorkflowPluginReview_CRUD(t *testing.T) {
	...
	err = db.Preload("Plugin").Preload("auth.User").First(&retrieved, review.ID).Error
	assert.NoError(t, err)
	assert.Equal(t, plugin.ID, retrieved.PluginID)
