...
func (qh *QdrantHandler) Query(ctx context.Context, text string, opts *QueryOptions) ([]QueryResult, error) {
	...
	timing := queryTimingFromContext(ctx)
	embedT0 := time.Now()
	vecs, err := qh.Embedder.Embed(ctx, []string{text})
	if timing != nil {
		timing.EmbedMs = time.Since(embedT0).Milliseconds()
	}
	if err != nil {
		return nil, err
	}
	...
	// Lean payload: title/content/source are enough for voice RAG; skip nested metadata blobs.
	reqBody := map[string]any{
		"vector":          qvec,
		"limit":           topK,
		"score_threshold": minScore,
		"with_payload": map[string]any{
			"include": []string{"title", "content", "source", "tags", "created_at", "updated_at"},
		},
	}
	if filter != nil {
	...
	if err := qh.doJSON(ctx, http.MethodPost, reqURL, reqBody, &resp); err != nil {
		if timing != nil {
			timing.SearchMs = time.Since(searchT0).Milliseconds()
		}
		// Surface dimension mismatches clearly (e.g. switched embed model without re-index).
		errMsg := err.Error()
		if strings.Contains(errMsg, "dimension") || strings.Contains(errMsg, "dim:") {
			return nil, fmt.Errorf("qdrant vector dim mismatch for collection %q (query dim=%d): %w — re-ingest KB after changing EMBED_*", collection, len(qvec), err)
		}
		return nil, err
	}
	if timing != nil {
