...
func createOPUSDecode(src, pcm media.CodecConfig) media.EncoderFunc {
	...
	// Decoder channel count: for normal voice+ASR with mono PCM and OPUS/48000/2, libopus often
	...
	if pcmOutCh >= 2 {
		...
		// PCM bridge: mono intermediate but peer negotiated stereo Opus — decode 2ch then downmix.
		decodeCh = 2
	}
	...
	return func(packet media.MediaPacket) ([]media.MediaPacket, error) {
		...
		if decodeCh == 2 {
			if pcmOutCh >= 2 {
				// Stereo PCM out (e.g. media bridge OPUS/48000/2 ↔ OPUS/48000/2): keep L/R.
...
func createOPUSEncode(src, pcm media.CodecConfig) media.EncoderFunc {
	...
	// AppVoIP suits interactive speech (voice/phone); AppAudio is broader-band music-oriented.
