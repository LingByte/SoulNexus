...
func (tl *TransportManager) processIncoming() {
	log.WithFields(map[string]interface{}{"sessionID": tl.session.GetSession().ID, "transport": tl.transport}).Info("input transport processing started")
	tl.incomingClosedChan = make(chan struct{}, 1)
	defer func() {
		if r := recover(); r != nil {
			log.WithFields(map[string]interface{}{"sessionID": tl.session.GetSession().ID, "transport": tl.transport, "error": r, "stacktrace": string(debug.Stack())}).Error("input transport processing panic")
		}
...
inputLoop:
	...
	log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": transport}).Warn("input transport processing ended")
}
...
func (tl *TransportManager) processOutgoing() {
	...
	defer func() {
		if r := recover(); r != nil {
			log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport, "error": r, "stacktrace": string(debug.Stack())}).Error("output transport processing panic")
		}
		// Always signal termination to unblock cleanup.
		select {
	...
	log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport}).Info("output transport processing started")
outputLoop:
	for {
		...
		case <-tl.session.ctx.Done():
			log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport}).Info("output transport processing canceled")
			break outputLoop
		case packet, ok = <-tl.txqueue:
			if !ok || packet == nil {
				log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport}).Info("output transport queue closed")
				break outputLoop
			}
		}
	...
	log.WithField("sessionID", tl.session.ID).Warn("output transport processing ended")
}
...
func (tl *TransportManager) cleanup() {
	...
	log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport}).Info("transport layer cleaned up")
}
...
func (tl *TransportManager) trySendPacket(packet MediaPacket) {
	...
	case tl.txqueue <- packet:
	default:
		log.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "packet": packet}).Info("packet dropped")
	}
}
...
type MediaSession struct {
	...
	// Serve's transport cleanup so another reader (e.g. voice transfer RTP relay) can own the UDP socket.
	...
	s.eventBus.Subscribe(EventTypePacket, func(ctx context.Context, event *MediaEvent) error {
		...
		for _, processor := range processors {
			if err := processor.Process(ctx, s, event); err != nil {
				log.WithFields(map[string]interface{}{"processor": processor.Name(), "sessionID": s.ID, "error": err}).Error("processor error")
...
func (s *MediaSession) AddOutputTransport(tx MediaTransport, filterFuncs ...PacketFilter) *MediaSession {
	...
	log.WithFields(map[string]interface{}{"sessionID": s.ID, "transport": tx, "queueSize": queueSize}).Info("output transport registered")
	tx.Attach(s)
...
func (s *MediaSession) setupOutputRouter() {
	...
	outputProcessor := NewPacketProcessor(
		...
		func(ctx context.Context, session *MediaSession, packet MediaPacket) error {
			// voice AI voice: do not loop decoded uplink PCM back to the caller (no echo).
			if ap, ok := packet.(*AudioPacket); ok && !ap.IsSynthesized {
				if v, ok := session.Get(KeySuppressUplinkEcho); ok {
...
func (s *MediaSession) Serve() error {
	...
	defer func() {
		if err := recover(); err != nil {
			log.WithFields(map[string]interface{}{"error": err, "stacktrace": string(debug.Stack())}).Error("session recover err")
			return
		}
		s.Running = false
		log.WithField("sessionID", s.ID).Info("session stopped")
	...
	if s.MaxSessionDuration > 0 {
		time.AfterFunc(time.Duration(s.MaxSessionDuration)*time.Second, func() {
			log.WithFields(map[string]interface{}{"sessionID": s.ID, "timeout": s.MaxSessionDuration}).Info("session stopped timeout")
	...
	s.EmitState(s, Begin)
	log.WithField("sessionID", s.ID).Info("session started")
...
func (s *MediaSession) CauseError(sender any, err error) {
	...
	if err != nil {
		...
		if strings.Contains(msg, "use of closed network connection") || strings.Contains(msg, "connection is closed") {
			log.WithFields(map[string]interface{}{"sessionID": s.ID, "sender": sender, "error": err}).Debug("cause error ignored (normal close)")
		} else {
			log.WithFields(map[string]interface{}{"sessionID": s.ID, "sender": sender, "error": err}).Error("cause error")
		}
	}
...
func (s *MediaSession) EmitState(sender any, state string, params ...any) {
	...
	log.WithFields(map[string]interface{}{"sender": sender, "state": state, "params": params, "sessionID": s.ID}).Info("emitstate")
...
func callHandleWithState(s *MediaSession, handle StateChangeHandler, state StateChange) {
	defer func() {
		if r := recover(); r != nil {
			log.WithFields(map[string]interface{}{"sessionID": s.ID, "state": state, "error": r, "handle": handle, "stacktrace": string(debug.Stack())}).Error("state panic")
		}
	}()
	handle(state)
}
...
func callHandleWithMediaData(s *MediaSession, h MediaHandler, handle MediaHandlerFunc, data MediaData) {
	defer func() {
		if r := recover(); r != nil {
			log.WithFields(map[string]interface{}{"sessionID": s.ID, "data": data, "error": r, "handle": handle, "stacktrace": string(debug.Stack())}).Error("handle panic")
		}
	}()
	handle(h, data)
}
...
func CastOption[T any](options map[string]any) (val T) {
	...
	if err != nil {
		log.WithFields(map[string]interface{}{"options": options, "target": reflect.TypeOf(val).Name(), "error": err}).Error("cast option error")
	}
	return
}
