...
const (
	// KeySuppressUplinkEcho: when true, output-router does not send decoded uplink
	// microphone AudioPackets to RTP output; only synthesized (TTS) audio is sent.
	KeySuppressUplinkEcho = "suppress_uplink_echo"
)
...
// Emitted by media RTP input when the peer sends out-of-band DTMF; not passed through audio codecs.
...
type CodecConfig struct {
	...
	// OpusPCMBridgeDecodeStereo: voice two-leg PCM bridge only — SDP OPUS/48000/2 payloads use a stereo
