...
// of the existing three layers is a category mistake — the voice
...
// block in callbacks; voice attach sites push events into channels handled by
// dedicated workers.
...
// this enum so voice integration code is provider-agnostic.
type EventType string

const (
	// EventSessionOpen fires once after a successful WS handshake +
	// session.update ack. Used by voice to mark "ready to forward audio".
	...
	// caller began speaking. voice must immediately stop forwarding any
	...
	// the current assistant response. Used by voice for hangup-phrase /
	...
	// session is unrecoverable and voice should fall back to playback.
...
type Options struct {
	...
	// equivalent boundary signals (advanced; not used by voice attach
	// today). Defaults to true.
	DisableServerVAD bool
	// Modalities selects which output streams the model emits. Empty =
	// vendor default (audio + text). The realtime voice path always wants
...
// multiple goroutines — voice attach uses a single audio-feed goroutine.
