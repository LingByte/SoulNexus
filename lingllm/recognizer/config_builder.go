...
func buildQCloudConfig(config map[string]interface{}) (*QCloudASROption, error) {
	...
	if v := cfg.Int("vadSilenceTime", "vad_silence_time", 0); v > 0 {
		opt.VadSilenceTime = v
	}
	if _, ok := config["needVad"]; ok {
		opt.NeedVad = cfg.Int("needVad", 0)
	} else if _, ok := config["need_vad"]; ok {
		opt.NeedVad = cfg.Int("need_vad", 0)
		if opt.NeedVad == 0 {
			opt.NeedVad = -1 // explicit 0 → force off
		}
	}
	return &opt, nil
}
...
func buildVolcengineConfig(config map[string]interface{}) (*VolcengineOption, error) {
	...
	cluster := cfg.String("cluster", "volcengine_input_common")
	...
	opt.Url = url
	if v := cfg.Int("endWindowSize", "end_window_size", "vadSilenceTime", "vad_silence_time", 0); v > 0 {
		opt.EndWindowSize = v
	}
	return &opt, nil
}
...
func buildVolcengineLLMConfig(config map[string]interface{}) (*VolcengineLLMOption, error) {
	...
	if rid := cfg.String("resourceId", "resource_id"); rid != "" {
		opt.ResourceId = rid
	}
	return &opt, nil
}
