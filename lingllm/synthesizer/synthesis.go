...
func (player *AudioSynthesisPlayer) Run(handler media.MediaHandler, ctx context.Context) {
	...
	frameSize := ComputeSampleByteCount(player.Format.SampleRate, player.Format.BitDepth, player.Format.Channels) * int(player.Format.FrameDuration.Milliseconds())
	st := time.Now()
	for {
		select {
		case <-ctx.Done():
...
func NewAudioSynthesisEngineFromCredential(config TTSCredentialConfig) (AudioSynthesisEngine, error) {
	...
	case "xunfei":
		...
		if resID := config.getString("resId"); resID != "" {
			xunfeiConfig.ResID = resID
		} else if resID := config.getString("res_id"); resID != "" {
			xunfeiConfig.ResID = resID
		}
		if vcn := config.getString("vcn"); vcn != "" {
			xunfeiConfig.Vcn = vcn
		}
		// 将配置对象转换为 map[string]any
