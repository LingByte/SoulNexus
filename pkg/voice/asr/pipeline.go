...
// It mirrors the hardware ASR pipeline shape for voice sessions:
...
type Options struct {
	...
	// (typical when lingllm/media/MediaSession has already decoded RTP).
	...
	// Optional audit metadata for ai_invocation_logs.
...
func New(options Options) (*Pipeline, error) {
	if options.ASR == nil {
		return nil, fmt.Errorf("voice/asr: ASR service is required")
	}
	lg := options.Logger
	if lg == nil {
	...
	p := &Pipeline{
		...
		metrics:     &Metrics{},
		logger:      lg,
		invTenantID: options.InvocationTenantID,
		invCallID:   strings.TrimSpace(options.InvocationCallID),
		invProvider: strings.TrimSpace(options.InvocationProvider),
		invSource:   strings.TrimSpace(options.InvocationSource),
	}
	if p.invSource == "" {
		p.invSource = "voice"
	}
...
func (p *Pipeline) ProcessPCM(ctx context.Context, pcm []byte) error {
	if p != nil && p.hasDecode {
		return fmt.Errorf("voice/asr: DecodeSource is set; use Process or ProcessEncoded with RTP payload bytes")
	}
...
func (p *Pipeline) ProcessEncoded(ctx context.Context, rtpPayload []byte) error {
	if p == nil || !p.hasDecode {
		return fmt.Errorf("voice/asr: no DecodeSource configured; use ProcessPCM")
	}
...
func (p *Pipeline) Process(ctx context.Context, audio []byte) error {
	if p == nil {
		return fmt.Errorf("voice/asr: nil pipeline")
	}
	if len(audio) == 0 {
		return nil
	}
...
func (p *Pipeline) onASRResult(text string, isFinal bool, _ time.Duration, _ string) {
	...
	if isFinal && (p.invCallID != "" || p.invTenantID > 0) {
		m := p.GetMetrics()
		entry := callbinding.AIInvocationRecord{
			TenantID:     p.invTenantID,
			Component:    callbinding.AIComponentASR,
			Provider:     p.invProvider,
			CallID:       p.invCallID,
			Source:       p.invSource,
			Status:       callbinding.AIStatusOK,
			LatencyMs:    m.ASRLatency.Milliseconds(),
			InputChars:   utf8.RuneCountInString(text),
			AudioBytes:   int64(m.TotalAudioBytes),
			ResponseText: text,
		}
		if m.ASRLatency > 0 {
			entry.AudioMs = m.ASRLatency.Milliseconds()
		}
...
func (p *Pipeline) onASRError(err error, fatal bool) {
	...
	if p.onErr != nil {
		...
		p.logger.Error("voice/asr error", zap.Error(err), zap.Bool("fatal", fatal))
	}
}
