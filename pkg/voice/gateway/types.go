...
const (
	...
	// EvTransferRequest is emitted when the media peer issued a REFER
	...
	// the media URI from Refer-To (e.g. sip:agent42@pbx.example.com).
	// Dialog apps decide policy: ignore, allow, prompt the LLM to
	// confirm before transferring, dial a Web seat, ... — VoiceServer
	// auto-emits a NOTIFY 200 OK after this event is fired so the media
...
type Event struct {
	...
	// transfer.request
	// Target carries the media URI from the inbound REFER's Refer-To
