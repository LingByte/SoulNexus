...
func TestCounter_BasicAndLabels(t *testing.T) {
	r := NewRegistry()
	r.IncCounter("c", "help", map[string]string{"t": "websocket"})
	r.IncCounter("c", "help", map[string]string{"t": "websocket"})
	r.IncCounter("c", "help", map[string]string{"t": "xiaozhi"})
	r.AddCounter("c", "help", map[string]string{"t": "websocket"}, 5)

	var buf bytes.Buffer
	r.WritePromText(&buf)
	out := buf.String()
	if !strings.Contains(out, `c{t="websocket"} 7`) {
		t.Errorf("want c{t=websocket} 7, got:\n%s", out)
	}
...
func TestAppHelpers_SmokeTest(t *testing.T) {
	...
	for _, want := range []string{
		...
		`voiceserver_asr_errors_total{transport="websocket"} 1`,
