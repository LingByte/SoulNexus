...
var (
	errTTSNilInnerService   = errors.New("voice/tts: nil inner service")
	errTTSEmptyVoiceKey     = errors.New("voice/tts: empty VoiceKey")
	errTTSNilCachingService = errors.New("voice/tts: nil caching service")
)
...
// voice sessions. The CachingService instance, by contrast, is per-call because
...
func NewCachingService(inner Service, cfg CacheConfig) (*CachingService, error) {
	if inner == nil {
		return nil, errTTSNilInnerService
	}
	if strings.TrimSpace(cfg.VoiceKey) == "" {
		return nil, errTTSEmptyVoiceKey
	}
	if cfg.Cache == nil {
