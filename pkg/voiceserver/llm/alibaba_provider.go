	...
	if resp.Output.Text != "" {
		if msgResp, ok := parseAlibabaPayload(resp.Output.Text); ok && strings.TrimSpace(msgResp.Message) != "" {
			...
			*fullResponse += msgResp.Message
			...
			// Avoid speaking raw JSON payloads to callers. Bailian's app
			// endpoint streams cumulative chunks; intermediate fragments
			// like `{"message":"听` look like the start of a JSON object
			// and must NOT be forwarded to TTS — we wait for a complete
			// object to arrive (handled by the parseAlibabaPayload branch
			// above). Final-state JSON (starts AND ends with braces) is
			// also suppressed since by that point the message has already
			// been emitted via the parseAlibabaPayload path.
