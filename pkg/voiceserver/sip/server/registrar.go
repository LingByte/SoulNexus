...
// registerPasswordOK is true when REGISTER is allowed: cfg.RegisterStaticPassword empty, or header matches.
func (s *SIPServer) registerPasswordOK(msg *stack.Message) bool {
	if s == nil {
		return true
	}
	required := strings.TrimSpace(s.cfg.RegisterStaticPassword)
	if required == "" {
		return true
	}
	if msg == nil {
