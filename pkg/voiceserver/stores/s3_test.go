...
func TestIntegration_S3_CRUD(t *testing.T) {
	...
	// 1) Write
	if err := store.Write("test-bucket", key, bytes.NewBufferString(content)); err != nil {
		t.Fatalf("Write err: %v", err)
	}

	// 2) Exists should be true
	ok, err := store.Exists("test-bucket", key)
	if err != nil {
	...
	// 3) Read
	rc, size, err := store.Read("test-bucket", key)
	if err != nil {
	...
	// 4) PublicURL
	u := store.PublicURL("test-bucket", key)
	if u == "" {
	...
	// 5) Delete
	if err := store.Delete("test-bucket", key); err != nil {
...
func TestS3Store_PublicURL(t *testing.T) {
	...
	u1 := store1.PublicURL("test-bucket", "path/to/file.txt")
	...
	u2 := store2.PublicURL("test-bucket", "file.txt")
	...
	u3 := store3.PublicURL("test-bucket", "path/file.txt")
	...
	u4 := store4.PublicURL("test-bucket", "file.txt")
	...
	u5 := store5.PublicURL("test-bucket", "path/file.txt")
...
func TestS3Store_Exists_NonExistent(t *testing.T) {
	...
	ok, err := store.Exists("test-bucket", nonExistentKey)
	if err != nil {
