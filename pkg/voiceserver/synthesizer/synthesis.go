...
func (player *SynthesisPlayer) Run(handler media.MediaHandler, ctx context.Context) {
	...
	frameSize := media.ComputeSampleByteCount(player.Format.SampleRate, player.Format.BitDepth, player.Format.Channels) * int(player.Format.FrameDuration.Milliseconds())
	st := time.Now()
	for {
		select {
		case <-ctx.Done():
