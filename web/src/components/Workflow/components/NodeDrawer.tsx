      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-[210] flex max-h-[80vh] flex-col rounded-t-2xl bg-white shadow-2xl dark:bg-gray-800"
          >
            {/* 抽屉头部 */}
            <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
              ...
              <Button type="text" size="sm" icon={<X className="h-5 w-5" />} onClick={onClose} />
            </div>
            
            {/* 搜索框 */}
            <div className="border-b border-[var(--color-border-2)] px-4 py-3">
              <Input
                prefix={<Search className="h-4 w-4 text-[var(--color-text-3)]" />}
...
const NodeCard: React.FC<NodeCardProps> = ({
  ...
  <motion.button
    type="button"
    className="group flex w-full flex-col items-center gap-2 rounded-xl border border-[var(--color-border-2)] bg-white p-3 text-center shadow-sm transition-all hover:border-[rgb(var(--primary-3))] hover:shadow-md dark:bg-[var(--color-bg-2)]"
    whileHover={{ y: -2 }}
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
  >
    <div className="relative flex h-12 w-12 items-center justify-center">
      <div
        className="absolute inset-0 rounded-full opacity-20 blur-md transition-opacity group-hover:opacity-35"
      ...
      >
        <div className="scale-90">{getIconComponent(iconName)}</div>
      </div>
    </div>
    <span className="line-clamp-2 text-xs font-medium text-[var(--color-text-1)]">{label}</span>
  </motion.button>
)
