export type Language = 'zh' | 'en' | 'ja'

export const common: Record<Language, Record<string, string>> = {
  zh: {
    // 品牌名称
    'brand.name': 'SoulMy',

    // 导航
    'nav.docs': '文档',
    'nav.about': '关于',
    'nav.login': '登录',
    'nav.enterSystem': '进入系统',
    'nav.logout': '退出登录',
    'nav.loginRegister': '登录 / 注册',
    'nav.register': '注册',
    'login.subtitle': '使用邮箱和密码登录您的账户',
    'login.password': '密码',
    'login.fillRequired': '请填写邮箱和密码',
    'login.twoFactorRequired': '请输入两步验证码',
    'login.twoFactor': '两步验证码（如已启用）',
    'login.submitting': '登录中…',
    'login.backHome': '返回首页',
    'nav.sidebar.playground': '演练场',
    'nav.sidebar.smartAssistant': '智能体',
    'nav.sidebar.voiceTraining': '音色训练',
    'nav.sidebar.knowledgeBase': '知识库',
    'nav.sidebar.workflow': '工作流',
    'nav.sidebar.pluginMarket': '插件市场',
    'nav.sidebar.notification': '通知中心',
    'nav.sidebar.jsTemplate': '桌宠市场',
    'nav.sidebar.profile': '个人中心',
    'nav.sidebar.credential': '密钥管理',
    'nav.sidebar.billing': '账单管理',
    'nav.sidebar.groups': '组织管理',
    'nav.sidebar.deviceManagement': '设备管理',
    'nav.sidebar.overview': '数据概览',
    'nav.sidebar.currentOrganization': '当前组织',
    'nav.sidebar.selectOrganization': '选择组织',
    'nav.sidebar.announcements': '公告中心',
    // 主题
    'theme.toggle': '切换主题',
    'theme.light': '浅色模式',
    'theme.dark': '深色模式',
    'theme.expand': '展开',
    'theme.collapse': '折叠',

    // 语言
    'lang.select': '选择语言',
    'lang.zh': '中文',
    'lang.zhTw': '繁體中文',
    'lang.en': 'English',
    'lang.ja': '日本語',
    'lang.fr': 'Français',

    // 通用操作
    'common.loading': '加载中...',
    'common.save': '保存',
    'common.cancel': '取消',
    'common.delete': '删除',
    'common.edit': '编辑',
    'common.create': '创建',
    'common.update': '更新',
    'common.confirm': '确认',
    'common.input': '输入',
    'common.back': '返回',
    'common.next': '下一步',
    'common.previous': '上一步',
    'common.close': '关闭',
    'common.refresh': '刷新',
    'common.search': '搜索',
    'common.filter': '筛选',
    'common.all': '全部',
    'common.none': '无',
    'common.unknown': '未知',
    'common.success': '成功',
    'common.failed': '失败',
    'common.error': '错误',
    'common.warning': '警告',
    'common.info': '信息',
    'common.empty': '暂无数据',
    'common.noData': '暂无数据',
    'common.noResults': '无结果',
    'common.total': '总计',
    'common.page': '页',
    'common.prevPage': '上一页',
    'common.nextPage': '下一页',
    'common.status': '状态',
    'common.type': '类型',
    'common.name': '名称',
    'common.description': '描述',
    'common.time': '时间',
    'common.createdAt': '创建时间',
    'common.updatedAt': '更新时间',

    'products.backHome': '返回首页',
    'products.heroTitle': '产品目录',
    'products.heroSubtitle': '语音、通信与数据管理相关方案，点击进入详情了解能力与接入方式。',

    'fileUpload.clickToUpload': '点击上传',
    'fileUpload.orDragDrop': '或拖拽文件到此处',
    'fileUpload.maxSize': '最大 {size}MB，最多 {count} 个文件',
  },
  en: {
    // Brand
    'brand.name': 'SoulMy',

    // Navigation
    'nav.docs': 'Documentation',
    'nav.about': 'About',
    'nav.login': 'Login',
    'nav.enterSystem': 'Enter System',
    'nav.logout': 'Logout',
    'nav.loginRegister': 'Login / Register',
    'login.subtitle': 'Sign in with your email and password',
    'login.password': 'Password',
    'login.fillRequired': 'Please enter email and password',
    'login.twoFactorRequired': 'Please enter your two-factor code',
    'login.twoFactor': 'Two-factor code (if enabled)',
    'login.submitting': 'Signing in…',
    'login.backHome': 'Back to home',
    'nav.sidebar.playground': 'Playground',
    'nav.sidebar.smartAssistant': 'Agent',
    'nav.sidebar.voiceTraining': 'Voice Training',
    'nav.sidebar.knowledgeBase': 'Knowledge Base',
    'nav.sidebar.workflow': 'Workflow',
    'nav.sidebar.pluginMarket': 'Plugin Market',
    'nav.sidebar.notification': 'Notifications',
    'nav.sidebar.jsTemplate': 'Pet Market',
    'nav.sidebar.profile': 'Profile',
    'nav.sidebar.credential': 'Credentials',
    'nav.sidebar.billing': 'Billing',
    'nav.sidebar.groups': 'Organizations',
    'nav.sidebar.deviceManagement': 'Devices',
    'nav.sidebar.overview': 'Overview',
    'nav.sidebar.currentOrganization': 'Current Organization',
    'nav.sidebar.selectOrganization': 'Select Organization',
    'nav.sidebar.announcements': 'Announcements',
    // Theme
    'theme.toggle': 'Toggle Theme',
    'theme.light': 'Light Mode',
    'theme.dark': 'Dark Mode',
    'theme.expand': 'Expand',
    'theme.collapse': 'Collapse',

    // Language
    'lang.select': 'Select Language',
    'lang.zh': '中文',
    'lang.zhTw': '繁體中文',
    'lang.en': 'English',
    'lang.ja': '日本語',
    'lang.fr': 'Français',

    // Common Actions
    'common.loading': 'Loading...',
    'common.save': 'Save',
    'common.cancel': 'Cancel',
    'common.delete': 'Delete',
    'common.edit': 'Edit',
    'common.create': 'Create',
    'common.update': 'Update',
    'common.confirm': 'Confirm',
    'common.input': 'Input',
    'common.back': 'Back',
    'common.next': 'Next',
    'common.previous': 'Previous',
    'common.close': 'Close',
    'common.refresh': 'Refresh',
    'common.search': 'Search',
    'common.filter': 'Filter',
    'common.all': 'All',
    'common.none': 'None',
    'common.unknown': 'Unknown',
    'common.success': 'Success',
    'common.failed': 'Failed',
    'common.error': 'Error',
    'common.warning': 'Warning',
    'common.info': 'Info',
    'common.empty': 'No Data',
    'common.noData': 'No Data',
    'common.noResults': 'No Results',
    'common.total': 'Total',
    'common.page': 'Page',
    'common.prevPage': 'Previous',
    'common.nextPage': 'Next',
    'common.status': 'Status',
    'common.type': 'Type',
    'common.name': 'Name',
    'common.description': 'Description',
    'common.time': 'Time',
    'common.createdAt': 'Created At',
    'common.updatedAt': 'Updated At',

    'products.backHome': 'Back to home',
    'products.heroTitle': 'Our products',
    'products.heroSubtitle':
      'Solutions for voice, communication, and data—open a product to see capabilities and how to integrate.',

    'fileUpload.clickToUpload': 'Click to upload',
    'fileUpload.orDragDrop': 'or drag and drop',
    'fileUpload.maxSize': 'Max {size}MB, up to {count} files',
  },
  ja: {
    // ブランド
    'brand.name': 'SoulMy',

    // ナビゲーション
    'nav.docs': 'ドキュメント',
    'nav.about': 'について',
    'nav.login': 'ログイン',
    'nav.enterSystem': 'システムに入る',
    'nav.logout': 'ログアウト',
    'nav.loginRegister': 'ログイン / 登録',
    'login.subtitle': 'メールアドレスとパスワードでログイン',
    'login.password': 'パスワード',
    'login.fillRequired': 'メールとパスワードを入力してください',
    'login.twoFactorRequired': '二段階認証コードを入力してください',
    'login.twoFactor': '二段階認証コード（有効な場合）',
    'login.submitting': 'ログイン中…',
    'login.backHome': 'ホームに戻る',
    'nav.sidebar.playground': 'プレイグラウンド',
    'nav.sidebar.smartAssistant': 'エージェント',
    'nav.sidebar.voiceTraining': '音声トレーニング',
    'nav.sidebar.knowledgeBase': 'ナレッジベース',
    'nav.sidebar.workflow': 'ワークフロー',
    'nav.sidebar.pluginMarket': 'プラグインマーケット',
    'nav.sidebar.notification': '通知',
    'nav.sidebar.jsTemplate': 'ペットマーケット',
    'nav.sidebar.profile': 'プロフィール',
    'nav.sidebar.credential': '認証情報',
    'nav.sidebar.billing': '請求',
    'nav.sidebar.groups': '組織',
    'nav.sidebar.deviceManagement': 'デバイス',
    'nav.sidebar.overview': '概要',
    'nav.sidebar.currentOrganization': '現在の組織',
    'nav.sidebar.selectOrganization': '組織を選択',
    'nav.sidebar.announcements': 'お知らせ',
    // テーマ
    'theme.toggle': 'テーマ切り替え',
    'theme.light': 'ライトモード',
    'theme.dark': 'ダークモード',
    'theme.expand': '展開',
    'theme.collapse': '折りたたみ',

    // 言語
    'lang.select': '言語選択',
    'lang.zh': '中文',
    'lang.zhTw': '繁體中文',
    'lang.en': 'English',
    'lang.ja': '日本語',
    'lang.fr': 'Français',

    // 共通アクション
    'common.loading': '読み込み中...',
    'common.save': '保存',
    'common.cancel': 'キャンセル',
    'common.delete': '削除',
    'common.edit': '編集',
    'common.create': '作成',
    'common.update': '更新',
    'common.confirm': '確認',
    'common.input': '入力',
    'common.back': '戻る',
    'common.next': '次へ',
    'common.previous': '前へ',
    'common.close': '閉じる',
    'common.refresh': '更新',
    'common.search': '検索',
    'common.filter': 'フィルター',
    'common.all': 'すべて',
    'common.none': 'なし',
    'common.unknown': '不明',
    'common.success': '成功',
    'common.failed': '失敗',
    'common.error': 'エラー',
    'common.warning': '警告',
    'common.info': '情報',
    'common.empty': 'データなし',
    'common.noData': 'データなし',
    'common.noResults': '結果なし',
    'common.total': '合計',
    'common.page': 'ページ',
    'common.prevPage': '前のページ',
    'common.nextPage': '次のページ',
    'common.status': 'ステータス',
    'common.type': 'タイプ',
    'common.name': '名前',
    'common.description': '説明',
    'common.time': '時間',
    'common.createdAt': '作成日時',
    'common.updatedAt': '更新日時',

    'products.backHome': 'ホームへ戻る',
    'products.heroTitle': '製品一覧',
    'products.heroSubtitle': '音声・通信・データ管理に関するソリューションです。詳細で機能と連携方法をご確認ください。',

    'fileUpload.clickToUpload': 'クリックしてアップロード',
    'fileUpload.orDragDrop': 'またはファイルをここにドラッグ',
    'fileUpload.maxSize': '最大{size}MB、最大{count}ファイル',
  }
}