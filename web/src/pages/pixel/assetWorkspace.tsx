...
import { addAssetFromFile, assetToBlob, deleteAsset, getImageDimensions, listAssets, updateAsset, type LocalAsset } from '@/lib/pixel/localAssetStore'
import { loadProjectSnapshots, saveProjectSnapshot } from '@/lib/pixel/assetProject'
import { convertImageBlob, FORMAT_OPTIONS, zipBlobs } from '@/lib/pixel/imageExport'
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.length === 0 ? (
              <div className="sm:col-span-2 xl:col-span-3 rounded-2xl border border-dashed border-border/70 bg-background/70 p-8 text-center">
                <Empty preset="no-data" description="从左侧导入 PNG / GIF / 音频等文件。" />
              </div>
            ) : (
              filtered.map((asset) => (
          ...
          <div className="w-full max-w-2xl rounded-2xl border border-border/60 bg-background p-4 shadow-xl">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_280px]">
              <div className="rounded-2xl border border-border/60 bg-card/80 p-3">
                {previewUrl && previewAsset.mimeType?.startsWith('image/') ? <img src={previewUrl} alt="" className="max-h-[420px] w-full rounded-xl object-contain" /> : <Empty preset="no-data" description="当前素材暂无图像预览。" />}
              </div>
