...
const MailTemplateEditPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) {
        await updateMailTemplate(id, form)
        Message.success('更新成功')
      } else {
        await createMailTemplate(form)
        Message.success('创建成功')
      }
      navigate('/platform/mail-templates')
    } catch (e: any) {
  ...
  return (
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">{isEdit ? `编辑模板：${original?.name || ''}` : '新建邮件模板'}</h1>
          <p className="text-sm text-[var(--color-text-3)]">左侧编辑模板，右侧预览。占位符：{'{{Name}}'}</p>
        </div>
        <Space>
            <Button onClick={() => navigate('/platform/mail-templates')}>
