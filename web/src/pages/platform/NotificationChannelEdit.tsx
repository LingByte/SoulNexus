...
import { Button, Input, InputNumber, Switch, Select, Space, Card, Message, Typography } from '@arco-design/web-react'
...
const NotificationChannelEditPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) {
        await updateNotificationChannel(id, form)
        Message.success('更新成功')
      } else {
        await createNotificationChannel(form)
        Message.success('创建成功')
      }
      navigate('/platform/notification-channels')
    } catch (e: any) {
  ...
  return (
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">{isEdit ? `编辑渠道：${channelName}` : `新建${isEmail ? '邮件' : '短信'}渠道`}</h1>
          <p className="text-sm text-[var(--color-text-3)]">配置发送供应商凭据。密钥留空表示保持原值。</p>
        </div>
        <Space>
            <Button onClick={() => navigate('/platform/notification-channels')}>
              ...
              <div className="space-y-3">
                ...
                {form.driver === 'smtp' ? (
                  ...
                  <>
                    ...
                    <Field label="投递回调 URL">
                      <Input
                        readOnly
                        value={`${window.location.origin}/api/webhooks/sendcloud`}
                        addAfter={
                          <Button
                            type="text"
                            size="small"
                            onClick={() => {
                              void navigator.clipboard.writeText(`${window.location.origin}/api/webhooks/sendcloud`)
                            }}
                          >
                            复制
                          </Button>
                      ...
                      <Typography.Text type="secondary" style={{ fontSize: 12, display: 'block', marginTop: 4 }}>
                        在 SendCloud 控制台配置此 URL 以接收投递、打开、点击、退信等状态回调（无需鉴权）。
                      </Typography.Text>
                    </Field>
                  </>
                )}
