      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4 dark:border-neutral-800 dark:bg-neutral-900/50">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            ...
            <ArcoInput size="large" className="!h-10 !text-base ![&::placeholder]:text-base" type="date" value={draft.start} onChange={(val) => setDraft((d) => ({ ...d, start: val }))} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-gray-400">事件时间（止）</label>
            <ArcoInput size="large" className="!h-10 !text-base ![&::placeholder]:text-base" type="date" value={draft.end} onChange={(val) => setDraft((d) => ({ ...d, end: val }))} />
          </div>
          <div>
